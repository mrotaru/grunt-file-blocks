...
Block.prototype.updateFiles = function () {
    ...
    if(this.config.manuallyAddedFiles && this.config.manuallyAddedFiles.length)
      files = files.concat(this.config.manuallyAddedFiles);
...
Block.prototype.rebuildFiles = function () {
    ...
    this.files = _.flatten(_.pluck(maps, 'dest'));
    this.changed = true;
};
